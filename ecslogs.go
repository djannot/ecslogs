...
func main() {
  if len(os.Args) != 7 && len(os.Args) != 8 {
                log.Fatalf("Usage: %s <user> <host:port> <pattern> <input log> <# days> <pipe|file> <output file>", os.Args[0])
        }

  var err error
  if os.Args[6] == "file" {
    f, err = os.Create(os.Args[7])
    if err != nil {
      panic(err)
    }
  ...
  for _, ip := range strings.Split(string(outIp),"\n") {
    if ip != "" {
